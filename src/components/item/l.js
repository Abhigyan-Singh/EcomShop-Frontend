...
const Item = (props) => {
  ...
  return (
    <div className={componentClassName} {...rest} style={{height:125}}>
      {item.onSale && (
        <img className="cbn-item__ribbon" src={saleRibbon} alt="Sale" />
      )}
      <div className={gridView ? "cbn-item__media" : 'position-absolute top-0 start-0 pt-5'}>
      ...
      <div className={gridView ? "cbn-item__information" : "p-1"}>
        <div
          onClick={() =>
            navigate(`/item/${item.productId}`, { state: {item, listItems} })
          }
        >
          <div className={gridView ? "cbn-item__name" :"cbn-item__name position-absolute top-0 start-0 end-0 bottom-0 p-20 pt-5"}>
            <a href="" className="block">
              {item.productName}
            </a>
          </div>
          <div className={gridView ? "cbn-item__number" : "position-absolute start-0 end-0 top-0 pt-11 p-20"}>Item #: {item.productId}</div>
        </div>
        <div className="cbn-item__size float-right position-absolute top-0 end-50 pt-6 m-20">
          {item.sizeString} | {(0.99).toFixed(2)}{' '}
          / {item.sizeUom}
        </div>
      </div>
      <div className={gridView ? "cbn-item__pricing" : null}>
        <div className={gridView ? "cbn-item__price" : "cbn-item__price float-right position-absolute top-0 end-0 start-50 pt-5 p-20"}>$ {item.normalPrice?.toFixed(2)}</div>
        {item.onSale && (
      ...
      <div className="cbn-item__controls">
        ...
        <a key={item.id} className="float-right position-absolute top-0 end-0 pt-5 pr-5">
          <Counter2
            item={item}
