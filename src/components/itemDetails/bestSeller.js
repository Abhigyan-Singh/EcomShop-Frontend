...
import { getBestSellers } from 'services/item-details';
...
const BestSeller = (props) => {
  ...
  const fetchBestSellers = (productId) => {
    getBestSellers(productId).then((res) => {
      const result = res.data.map((each) => ({
        ...each.suggestedProduct,
        keywords: []
      }));
      setItems([...result]);
    });
